...
public class SP_DPM_GST_15A_COMM_TEMP_REPORT_Result {
	...
	public SP_DPM_GST_15A_COMM_TEMP_REPORT_Result generateTempResult(List<Object[]> resultList) {
		if (resultList.size() == 0)
			return null;
		else {
			List<SP_DPM_GST_15A_COMM_TEMP_REPORT_Result> list = new ArrayList<SP_DPM_GST_15A_COMM_TEMP_REPORT_Result>();
			for (Object[] objectArray : resultList) {
				SP_DPM_GST_15A_COMM_TEMP_REPORT_Result temp = new SP_DPM_GST_15A_COMM_TEMP_REPORT_Result();
	...
	private SP_DPM_GST_15A_COMM_TEMP_REPORT_Result generateReportBean(Object[] objectArray) {
		...
		if (null != objectArray[4]) {
			temp.setSUO_MOTO_OPENING_BALANCE(String.valueOf(objectArray[4]));
		}
		if (null != objectArray[5]) {
			temp.setSUO_MOTO_GSTIN_LIABLE_FOR_CANCELLATION(String.valueOf(objectArray[5]));
		}
		if (null != objectArray[6]) {
			temp.setSUO_MOTO_GSTIN_CANCELLED(String.valueOf(objectArray[6]));
		}
		if (null != objectArray[7]) {
			temp.setSUO_MOTO_GSTIN_PENDING_FOR_CANCELLATION(String.valueOf(objectArray[7]));
		}
		if (null != objectArray[8]) {
			temp.setCANCELLATION_OPENING_BALANCE(String.valueOf(objectArray[8]));
		}
		if (null != objectArray[9]) {
			temp.setCANCELLATION_NO_APPLICATION_RECEIVED(String.valueOf(objectArray[9]));
		}
		if (null != objectArray[10]) {
			temp.setCANCELLATION_GSTIN_CANCELLED(String.valueOf(objectArray[10]));
		}
		if (null != objectArray[11]) {
			temp.setCANCELLATION_GSTIN_PENDING_FOR_CANCELLATION(String.valueOf(objectArray[11]));
		}
		if (null != objectArray[12]) {
			temp.setTOTAL_PENDENCY(String.valueOf(objectArray[12]));
		}
		if (null != objectArray[13]) {
			temp.setREVOCATION_OPENING_BALANCE(String.valueOf(objectArray[13]));
		}
		
		if (null != objectArray[14]) {
			temp.setREVOCATION_ARN_RECEIVED(String.valueOf(objectArray[14]));
		}
		
		if (null != objectArray[15]) {
			temp.setREVOCATION_GSTIN_REVOKED(String.valueOf(objectArray[15]));
		}
		
		if (null != objectArray[16]) {
			temp.setREVOCATION_APPLICATION_REJECTED(String.valueOf(objectArray[16]));
		}
		
		if (null != objectArray[17]) {
			temp.setREVOCATION_NO_ARN_PENDING(String.valueOf(objectArray[17]));
		}
