     ...
     <%!  String desDt = ""; String desDt1 = ""; int month=0; int year=0; int rowNO=0; %>
        ...
        <c:if test="${!(empty Model)}">
                    ...
                    <td style="text-align: left;">
                        ...
                        <c:if test = "${AMND != 0}">
                            <a href="../../../../../Home/CorrectionBackup?MM_YYYY=${ViewBag_MM_YYYY}.MM_YYYY&MPR=DPM-ST-1AC" style="color:brown; font-weight:bold;"><img src="<%= session.getAttribute("DYNAMIC_LINK_PATH") %>Content-MPR/imagesMPR/mpr-img/Amended.gif" /></a>
                        </c:if>
                    </td>
                    <%
                        ...
                        <a download='MSR-DPM-ST-1AC(<%= desDt.toUpperCase() %>).xls' href="#">Download</a>
...
<c:set var = "opening_no"  value = "${opening_no + r.JC_OPENING_NO}"/>
<c:set var = "receipts_for"  value = "${receipts_for + r.JC_RECEIPT_NO_FOR}"/>
<c:set var = "receipts_upto"  value = "${receipts_upto + r.JC_RECEIPT_NO_UPTO}"/>
<c:set var = "disposal_for"  value = "${disposal_for + r.JC_DISPOSAL_NO_FOR}"/>
<c:set var = "disposal_upto"  value = "${disposal_upto + r.JC_DISPOSAL_NO_UPTO}"/>
<c:set var = "closing_no"  value = "${closing_no + r.JC_CLOSING_NO}"/>
<c:set var = "less_3"  value = "${less_3 + r.JC_MONTHS_3_AMT_FOR}"/>

<c:set var = "less_6"  value = "${less_6 + r.JC_MONTHS_3TO6_AMT_FOR}"/>
<c:set var = "less_12"  value = "${less_12 + r.JC_MONTHS_6TO12_AMT_FOR}"/>
<c:set var = "above_1"  value = "${above_1 + r.JC_MORE_YEAR_AMT_FOR}"/>
                    ...
                    <tr class="grid_head">
                        ...
                        <th align="left" width="25%" height="22px">
                         ...
                         <a id="Zone_Link" href= "<%= session.getAttribute("DYNAMIC_LINK_PATH") %>GSTDPM/MSR/MSR_DPM_ST_1AC_COMM/ALL/${r.ZONE_CODE}/DSC/<%= desDt1.toUpperCase() %>/<%= session.getAttribute("CSRF_Token") %>" >${r.ZONE_NAME}</a>
                        ...
                        <td align="right" width="7%" height="22px">
                            ${r.JC_RECEIPT_NO_UPTO}
                        </td>
                        <td align="right" width="7%" height="22px">
                            ${r.JC_DISPOSAL_NO_FOR}
                        </td>

                        <td align="right" width="7%" height="22px">
                            ${r.JC_DISPOSAL_NO_UPTO}
                        </td>
                        <td align="right" width="7%" height="22px">
                            ${r.JC_CLOSING_NO}
                        </td>
                        <td align="right" width="7%" height="22px">
                            ${r.JC_MONTHS_3_AMT_FOR}
                        </td>
                        <td align="right" width="7%" height="22px">
                            ${r.JC_MONTHS_3TO6_AMT_FOR}
                        </td>

                        <td align="right" width="7%" height="22px">
                            ${r.JC_MONTHS_6TO12_AMT_FOR}
                        </td>
                        <td align="right" width="7%" height="22px">
                            ${r.JC_MORE_YEAR_AMT_FOR}
                        </td>
