     ...
     <%!  String desDt = ""; String desDt1 = ""; int month=0; int year=0; int rowNO=0; %>
        ...
        <c:if test="${!(empty Model)}">
                    ...
                    <%
                        ...
                        <a download='MSR-DPM-ST-1AF(<%= desDt.toUpperCase() %>).xls' href="#">Download</a>
        ...
        <c:if test="${!(empty Model)}">
            ...
            <table class="Report_body" style="width: 70%;" align="center">
                ...
                <tr>
                    ...
                    <th align="center">
                        % OF CASES OUT OF TOTAL AMOUNT(AMT. OF ADJ CASES/TOTAL CASES)
                    ...
                    <tr class="grid_head">
                        ...
                        <c:if test="${r.ZONE_CODE != 'ZY'}">
                            <th align="left" width="4%" height="22px">
								...
								<a id="Zone_Link" href= "<%= session.getAttribute("DYNAMIC_LINK_PATH") %>GSTDPM/MSR/MSR_DPM_ST_1B_COMM/ALL/${r.ZONE_CODE}/DSC/<%= desDt1.toUpperCase() %>/<%= session.getAttribute("CSRF_Token") %>" >${r.ZONE_NAME}</a>
                            </th>
                       </c:if> 
                       <c:if test="${r.TOTAL_CLOSING_AMT_PER == 'ZY'}">
                            <th align="left" width="4%" height="22px">
                                ${ r.ZONE_NAME}
                            </th>
                       </c:if>
                        ...
                        <td align="right" width="4%" height="22px">
											...
											<c:if test="${r.TOTAL_CLOSING_AMT_PER == null}">
	                                                       0
	                                        </c:if> <c:if test="${r.TOTAL_CLOSING_AMT_PER != null}">
	                                           	${r.TOTAL_CLOSING_AMT_PER}
	                                        </c:if>
                        </td>
