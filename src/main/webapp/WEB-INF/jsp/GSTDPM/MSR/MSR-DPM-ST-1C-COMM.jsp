...
<c:if test="${total_oio_upto != 0}">
          <c:set var = "per_30"  value = "${(no_30_upto / total_oio_upto) * 100}"/>
                    ...
                    <tr class="grid_head">
                        ...
                        <td align="right" width="4%" height="22px">
                            <!-- @if (r.TOTAL_PERCENT_FOR == -999)
                            {
                                @Html.Raw("-");
                            }
                            else
                            {
                                @(string.Format("{0}%", r.TOTAL_PERCENT_FOR == null ? "0" : Html.Raw(r.TOTAL_PERCENT_FOR)))
                            } -->
                            
                           
	                         <c:if test="${r.TOTAL_PERCENT_FOR ==null||r.TOTAL_PERCENT_FOR == '-999.00'}">
	                               -
	                        </c:if> 
	                        	                        
	                        <c:if test="${r.TOTAL_PERCENT_FOR != '-999.00'&&r.TOTAL_PERCENT_FOR !=null}">
	                                ${r.TOTAL_PERCENT_FOR}
	                        </c:if>
                        ...
                        <td align="right" width="4%" height="22px">
                            ...
                            <!-- @if (r.TOTAL_PERCENT_UPTO == -999)
                            {
                                @Html.Raw("-");
                            }
                            else
                            {
                                @(string.Format("{0}%", r.TOTAL_PERCENT_UPTO == null ? "0" : Html.Raw(r.TOTAL_PERCENT_UPTO)))
                            } -->
                            
                           
	                         <c:if test="${r.TOTAL_PERCENT_UPTO ==null||r.TOTAL_PERCENT_UPTO == '-999.00'}">
	                               -
	                        </c:if> 
	                       
	                        <c:if test="${r.TOTAL_PERCENT_UPTO != '-999.00'&&r.TOTAL_PERCENT_UPTO !=null}">
 ...
 <a id="back_lnk" href="<%= session.getAttribute("DYNAMIC_LINK_PATH") %>GSTDPM/MSR/MSR_DPM_ST_1C/All/0/DSC/<%= desDt1.toUpperCase() %>/<%= session.getAttribute("CSRF_Token") %>">Back to Previous Report</a>
