...
<script language="javascript" type="text/javascript">
    ...
    function DisplayReport(group, data, divNo) {
    	...
    	var _month = $("#ddl_mth").val();
        var _year = $('#ddl_year').val();

        var _s_month = (new Date).getMonth()+1;
        var _s_year = (new Date).getFullYear();
        //alert(_month +"-"+_year);
        ...
        if (data != '[]' && data != '') {
            ...
            for (var x = 0; x < arr.length; x++) {
                ...
                if(arr[x].CODE=="DDM-GST-3" || arr[x].CODE=="DDM-GST-3A" || arr[x].CODE=="DDM-GST-4" || arr[x].CODE=="DDM-GST-4A" || arr[x].CODE=="GST-GST-2")
                {
                    if((_s_month-1 == _month) && (_s_year ==_year))
                    {
                    	markup += "<a  class='btn btn-default' title='This proforma contains data pertaining to Section 39 of the CGST Act, 2017 and is available for the month prior to the reporting month' style='margin:10px; background-color: #f2dede;'>" + arr[x].CODE + "</a>"
                    }
                       
                    else
                    {                    	
                    	markup += "<a  class='btn btn-default' title='This proforma contains data pertaining to Section 39 of the CGST Act, 2017 and is available for the month prior to the reporting month' style='margin:10px; background-color: #f2dede;' href='javascript: GetReport(" + '"' + arr[x].CODE + '"' + ");'>" + arr[x].CODE + "</a>" 
                    }
                	
                }
                else if(arr[x].CODE=="DDM-GST-1" || arr[x].CODE=="DDM-GST-2" || arr[x].CODE=="DDM-GST-5" || arr[x].CODE=="DPM-GST-4" || arr[x].CODE=="DPM-GST-12" || arr[x].CODE=="DPM-GST-14" || arr[x].CODE=="DPM-GST-15" || arr[x].CODE=="DPM-GST-ADJ-1" || arr[x].CODE=="DPM-GST-ADJ-2" || arr[x].CODE=="DPM-GST-ADJ-3")
                {
                	markup += "<a  class='btn btn-default' style='margin:10px; background-color: #f2dede;' href='javascript: GetReport(" + '"' + arr[x].CODE + '"' + ");'>" + arr[x].CODE + "</a>"
                }
