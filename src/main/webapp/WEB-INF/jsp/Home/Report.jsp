    ...
    <div class="row">
		...
		<div class="col-lg-6">
			...
			<ol class="breadcrumb float-right no-bg-col p-t-20 m-b-0 p-r-0">
				<li><a href="/PRE_KPI_Home"><i class="fa fa-home"></i> Management Information</a></li>
                    ...
                    <div class="panel-body">
                        <div class="row">
                            ...
                            <div class="col-lg-12">
                                ...
                                <div class="col-lg-2">
                                    <div class="form-group">
                                        <select id="ddlregin" name="ddlregin" class="form-control" onchange = "javascript:HideMPR(this.value);"></select>
...
<script language="javascript" type="text/javascript">
    $(document).ready(function () {
        ...
        $('#ddlregin').hide();
        $("#div_1 , #div_2 , #div_3, #div_4, #div_5, #div_6, #div_7, #div_8, #div_9").hide();
    ...
    function GetZone(_stateId) {
        ...
        var url = "<%=request.getSession().getAttribute("DYNAMIC_LINK_PATH") %>Home/Zone_Bind/";
        //var url = "/Home/Zone_Bind/";
    ...
    function GetComm(zoneId, dep) {
        ...
        var url = "<%=request.getSession().getAttribute("DYNAMIC_LINK_PATH") %>Home/Comm_Bind/";
 ...
 /*   function GetReport(_reportname)
    {
        ...
        if ($('#ddlzone').val() == '00')
        {
            window.open("/MIS/" + _reportname.substring(0, 3) + "/Report/" + _reportname.replace(/\-/g, '_') + "/Region/" + $('#ddlregin').val() + "/" + $("#ddlregin option:selected").text() + "/" + $("#ddl_mth option:selected").text() + "-" + $('#ddl_year').val(), '_blank');
            return false;
        }
        var url = "/Home/Report_Redirect/";
    ...
    */
    ...
    function GetReport(_reportname) {
        ...
        var url = "<%=request.getSession().getAttribute("DYNAMIC_LINK_PATH") %>Home/Report_Redirect/";
        var _dep = $('#ddl_dep').val();
        var _zone_code = $('#ddlzone').val();
        var _zone_name = $("#ddlzone option:selected").text();
    ...
    function ReportBind() {
        ...
        var url = "<%=request.getSession().getAttribute("DYNAMIC_LINK_PATH") %>Home/ReportBind/";
        $.ajax({
            url: url,
            data: {
                comm: _comm,
            ...
            success: function (data) {
                ...
                if ($('#ddlzone').val() == '0A') {
                    ...
                    $("#part_1").html(DisplayReport('Part-I Revenue', data.data1, 1)).show();
                    $("#part_2").html(DisplayReport('Part-II Anti-smuggling', data.data2, 2)).show();
    ...
    function DisplayReport(group, data, divNo) {
        if (data != '[]' && data != '') {
            ...
            //markup += "<tr><td colspan='4'><a href='javascript: Submit_Report(" + '"' + group + '"' + ");'>Submit</a></td></tr>"
    ...
    function Get_MemberReport() {
        if ($("input:radio[name='Report_Type']:checked").val() == "FF") {
            if ($('#ddlzone').val() == "00") {
                if ($('#ddlregin').val() == "01" || $('#ddlregin').val() == "02" || $('#ddlregin').val() == "03" || $('#ddlregin').val() == "04" || $('#ddlregin').val() == "05" || $('#ddlregin').val() == "06") {
                    ReportEmpty();
                    $("#btn1").show();
    ...
    function redirectToMemberReport() {
        var data = '/Home/MemberReport?RegionCode=' + $('#ddlregin').val() + '&RegionName=' + $('#ddlregin option:selected').text() + '&Month=' + $('#ddl_mth option:selected').text() + '&Year=' + $('#ddl_year').val();
        window.location.href = data;
    }
    jQuery('#ddl_year').change(function (data) {
        var selectedYear = jQuery(this).val();
        var data = "";
        data += "<option value= >Select Month</option>";
        
        if (selectedYear == '2017') {
            data += "<option value='1' >JAN</option>";
            data += "<option value='2' >FEB</option>";
            data += "<option value='3' >MAR</option>";
            data += "<option value='4' >APR</option>";
            data += "<option value='5' >MAY</option>";
            data += "<option value='6' >JUN</option>";
        }
        else {
            data += "<option value='4' >APR</option>";
            data += "<option value='5' >MAY</option>";
            data += "<option value='6' >JUN</option>";
            data += "<option value='7' >JUL</option>";
            data += "<option value='8' >AUG</option>";
            data += "<option value='9' >SEP</option>";
            data += "<option value='10' >OCT</option>";
            data += "<option value='11' >NOV</option>";
            data += "<option value='12' >DEC</option>";
            data += "<option value='1' >JAN</option>";
            data += "<option value='2' >FEB</option>";
            data += "<option value='3' >MAR</option>";
        }
